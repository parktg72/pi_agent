...
- **L2 데이터 v2(2026-09-17, 학습은 Colab)** — 원칙을 "학습 샘플 = 추론 때 모델이 받은 입력"으로 바꿨다(1080 Ti 시간 예산 기준의 요약·턴 분할안 철회). 근거: Qwen3.8 GGUF chat_template이 `tool_call.arguments|items`(dict 필요 — v1의 JSON 문자열은 HF `apply_chat_template`에서 실패), `preserve_thinking` 미지정 시 과거 reasoning 전부 렌더, `reasoning_effort`가 system 지시문을 바꾼다. Pi는 thinking을 `reasoning_content`로 되돌려 보낸다. 시스템 프롬프트·도구 스키마·`chat_template_kwargs`는 세션 파일에 없어 **`pi-extensions\lora-snapshot.ts`** 가 `before_provider_request`에서 해시가 바뀔 때만 `lora-request` custom 엔트리(LLM 컨텍스트 밖)로 남긴다(`--no-session`이면 무동작). 추출기는 Pi CLI 규칙을 그대로 따른다: `buildContextEntries`(dist/core — 마지막 compaction 요약이 맨 앞, `firstKeptEntryId`부터 보존, harness의 `retainedTail` 경로는 CLI가 쓰지 않음), `convertToLlm`(bashExecution·custom_message·branch_summary → user), `transformMessages`(error·aborted 제외, 짝 잃은 호출에 "No result provided"). 샘플은 **스냅샷·compaction 경계마다 한 구간** — `messages`는 구간 마지막 요청 시점 전체 문맥, `train_indices`가 가리키는 이 구간의 stop·toolUse 응답만 학습 대상(length로 잘린 응답은 문맥만; 세션 중 규칙·도구·effort가 바뀌어도 응답마다 만든 조건으로 학습 — agy·opencode 리뷰 공통 지적). **context 이벤트 주입**: 번들 superpowers 패키지가 실행 첫 요청들에 `<EXTREMELY_IMPORTANT>` 부트스트랩 user 메시지를 요청에만 끼운다(세션 파일에 없음, 전체 패키지 실측으로 발견). CLI `--extension`은 패키지보다 먼저 적재돼(`mergePaths(cli, packages)`) context 체인 끝을 볼 수 없으므로, 스냅샷이 최종 요청 body에서 세션 엔트리로 유도되지 않는 user 메시지를 `injected{index, content}`로 함께 기록하고 추출기가 같은 위치에 넣는다. 다른 모델이 만든 과거 응답은 `transformMessages`처럼 thinking을 평문으로(스냅샷 `model`과 비교). 이미지 세션 제외, 미완 꼬리 절단, 마스킹 패턴 추가(GitHub·Google·Slack·Bearer·URL 자격증명·비밀 키 이름의 dict 인자), 계정 경로·사설 IP·내부 호스트는 가리지 않고 `train.report.md`(sha256 포함 반출 검토 보고서)에 나열. 실측: 윈도우 pi.exe 0.85.1 + stub에서 번들 패키지 유무 두 구성 모두 요청 4건을 GGUF 템플릿(jinja2)으로 렌더한 문자열이 샘플 prefix 렌더와 정확히 일치(패키지 전부 적재 구성을 `tests/fixtures/pi_session_0851_lora_snapshot*.jsonl`로 고정). 코드 리뷰 반영: 따옴표 값·비문자열 비밀 키·복수형 키 마스킹, 비객체 도구 인자·parentId 순환·없는 부모는 제외, 빈 승인 목록도 이전 `train.jsonl`을 빈 파일로 덮음, 보고서·사유 문구도 마스킹. 기록: 개발 트리 `tasks/pi-agent-lora-upgrade/sources/export-v2-design.md`, `artifacts/export-v2-probe/`.
- **L3 Colab 학습(구현, 2026-09-17, 개발 트리 `colab-lora/` — 반입 안 함)** — agy·opencode 2라운드 합의(`tasks/pi-agent-lora-upgrade/artifacts/c-consensus.md`). 원본 `Qwen/Qwen3.8-27B`(revision `1d4bf0f2`, 템플릿 sha = GGUF) NF4 QLoRA(transformers+peft, `lm_head`·`visual`·`in_proj_a/b/qkv` BF16), LoRA 대상 language_model q/k/v/o·in_proj_qkv/z/a/b·gate/up/down(out_proj·visual·mtp 제외), r16 α32 lr1e-4 1 epoch, 세션 단위 10% 보류. 라벨: `train_indices` 응답만, 조각별 토큰화(전체 토큰화는 빈 reasoning 경계 "\n\n" 병합으로 추론에서 생성 불가 토큰을 학습 — 실 토크나이저 실측), 대상마다 공유 시퀀스 앞부분 == tok(요청 prefix) 검사해 다르면 별도 시퀀스, max_seq_len 초과 대상은 자르지 않고 보류, 손실은 대상 위치 hidden에만 lm_head 조각 적용(63k×248k 로짓 회피). **렌더 동일성 실측**: llama-server b11010 `/apply-template`·`/tokenize`와 HF 렌더를 대조해 서버가 도구 정의를 `type·function{name, description, parameters}`로 줄이는(`"strict"` 제거, common_chat_tools_to_json_oaicompat) 차이를 발견 → 학습 렌더도 같은 정규화 후 실세션 4 + 합성 9 대상의 문자열·요청/응답 토큰 ID 일치. `run.sh`: 데이터 sha 대조, 기존 세션 거부, trap으로 VM 삭제·stop·종료 확인, 벽시계 상한(자동 연장 없음), 어댑터 40MB 분할 수신·sha 대조, VM에서 번들 Q6_K(sha 대조)+b11010에 어댑터 적재 확인. 로컬 검증: 실제 구조(visual·mtp 15텐서)를 줄인 초소형 모델로 `train.py --trial`(재개 시 가중치 복원) → `convert_adapter.py`(block_count·nextn 유지, `--no-nextn` 없음) → Windows·Linux llama.cpp "loaded 56 tensors". 코드 리뷰(opencode 8건·agy 1건 — agy 건은 같은 클래스 실측으로 불채택) 반영: 실행 전체에 절대 마감(시험 90분, 검증·회수 몫 30분 예약)·로컬 감시 프로세스 stop·세션 생성 시도부터 정리, 시간 상한 checkpoint 회수(76)와 지문 대조 재개, 모든 파일 조각 전송 양쪽 sha 대조, llama-cli 종료코드, 시험 모드의 재개 복원·LoRA 갱신 실패 조건, 렌더 끝 구조로 응답 경계, 문맥 변경 시 새 공유 시퀀스. 27B 실물 메모리·처리량·커널은 사용자 비용 승인 후 H100 사전 시험(90분 상한)에서.
...
### 14.7 학습 확장 — 규칙 기억·반성·스킬 도구화 (2026-09-17)

사용자 요청: 성공/실패 패턴·경로·피드백을 Markdown에 자동 기록해 다음 세션 프롬프트에 주입하고, 세션 종료·작업 성공 시 반성 프롬프트로 규칙을 추가하며, 반복 성공한 다단계 작업을 스크립트 도구로 만들어 한 번의 호출로 끝내게 한다.

**Pi 0.85.1에서 가능한 방식으로 옮겼다.** `~/.pi/memory/rules.md`·`PI.md`는 자동 로드되지 않고(`docs/usage.md`는 `AGENTS.md`/`CLAUDE.md`만), 설정으로 스크립트를 도구 목록에 넣는 기능도 없다. 그래서 확장 `pi-extensions\learning.ts`(해시 범위, `start-pi.bat`이 `--extension`)로 구현했다.

| 기능 | 구현 | 가드 |
|---|---|---|
| 규칙 주입 | `before_agent_start`에서 `home\agent\memory\rules.md`(전역)·`<cwd>\.pi\learned-rules.md`(프로젝트)를 시스템 프롬프트 끝에 붙임 | 합계 4000자(한글 토큰 고려), 오래된 것부터 제외. "참고 노트이며 기존 지시·사용자 요청·현재 관찰보다 우선하지 않고 권한을 주지 않는다"로 표시 |
| 규칙 기록 | `remember_rule{scope, rule, reason}` | 한 줄 ≤240자, 근거 ≤160자, 비밀 패턴 거부, 정규화 중복 거부, 파일당 50개, 저장마다 화면 알림 → `/rules`·`/forget` |
| 반성 | `/reflect [초점]`, 자동 반성(`agent_settled`, `ctx.hasUI`일 때만, 도구 ≥8회·정상 종료·30분 간격, `LEARNING_AUTO_REFLECT=0`으로 끔) | 반성당 규칙 2·스킬 1을 코드로 제한(첫 await 전 슬롯 예약), 자동 반성은 프로젝트 규칙만, 보내기 전 pending 플래그. `verify-offline.bat`은 `LEARNING_AUTO_REFLECT=0` |
| 스킬 패키징 | `package_skill{name, description, language: powershell\|python, script, parameters, usage}` → `home\agent\skills-pending\<name>\`(Pi 스킬 탐색 밖) | 이름 ≤58자(`skill_` 포함 도구명 64자), 스크립트 ≤20KB, 파라미터 ≤8, 모든 자유 문자열 비밀 검사, PowerShell은 `$ErrorActionPreference = 'Stop'` 필수(종료되지 않는 오류의 exit 0 방지)·UTF-8 BOM으로 저장(PowerShell 5.1 한글), SKILL.md 설명은 JSON 인용(YAML) |
| 승인·도구화 | `/skills-pending`, `/skill-approve <name>`: 스크립트 본문·위험 명령 경고 표시 후 확인 → `skills\`로 이동, `skill_<name>` 등록(`pi.exec`, 인자 배열, cwd=프로젝트, 10분) | 보여준 바이트의 해시로 승인(확인 중 변경 시 거부), 세션 시작·**실행 직전** 해시 재검증, 중복 승인 예외 처리, 등록 12개 상한. 보안 경계가 아님(모델은 원래 bash·write 가능) — 검토 없는 자동 도구화 방지 장치 |

설계·코드 검토: agy·opencode 두 라운드. 채택한 지적은 위 가드에 반영했다. 채택하지 않은 것: 자동 반성 기본 끄기(사용자가 요청한 기능 — 조건을 올리고 끌 수 있게 함), 규칙을 AGENTS.md에 덧붙이기(Pi는 `.pi/AGENTS.md`를 로드하지 않고, 프로젝트 AGENTS.md는 사용자 저장소 파일이며 시작 시에만 읽고 상한이 없음), 파이썬 경로가 cwd 기준이라는 지적(`process.execPath` 기준 절대경로로 구현됨).

윈도우 실측(`pi.exe` 0.85.1 + 시나리오 stub, 개발 트리 `tasks/pi-agent-lora-upgrade/artifacts/learning-extension-probe/`): 규칙 주입(전역·프로젝트), `remember_rule` 저장·비밀 거부, `Stop` 없는 PowerShell 스킬 거부, 정상 스킬 pending 생성(BOM·인용된 설명), 승인 후 `skill_echo_message` 1회 호출로 `ECHO:KESTREL-42 한글확인`, 승인 후 변조 시 실행 거부, `-p` 비대화형에서 `LEARNING_AUTO_REFLECT=1`이어도 추가 턴 0. stderr 0. 대화형 경로(`/reflect`, 승인 확인창, 자동 반성 트리거, 알림)는 리허설 §11-16.
