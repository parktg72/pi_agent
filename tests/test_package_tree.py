...
def test_a_file_only_in_the_destination_is_a_warning_not_a_failure(tmp_path):
    # xcopy /D는 상류에서 삭제된 파일을 지우지 않고, home\\agent\\는 Pi 자신도
    # 쓰는 가변 영역이다. 실패로 다루면 정상 운영이 빨간불이 된다.
    source, destination = make_pair(tmp_path)
    (destination / "node_modules" / "leftover.js").write_text("old")
    failures, warnings = package_tree.compare_trees(source, destination)
    assert failures == []
    assert any("대상에만 있음" in warning for warning in warnings)
